...
def main():
    ...
    interval_ms = 2  # Request interval in milliseconds (controls CAN bus communication rate)
    ...
    import argparse
    parser = argparse.ArgumentParser(description='OmniHand 2025 Tactile Sensor Raw Data Reliability Test')
            ...
            for finger, sensor_name, expected_length in sensor_positions:
                try:
                    ...
                    if sensor_data is None:
                        # No sensor data object returned
                    ...
                    # Exception during data acquisition
                    failure_count[sensor_name] += 1
                    total_failures += 1
                    if iteration < 10:  # Only print first few errors to avoid spam
                        # Print error on new line, then continue with progress bar
                        print(f"\n[Iteration {iteration}] Error getting {sensor_name} data: {e}")
            
            # Calculate total time for querying all 7 sensors
