...
@Date: 2025-11-06
@Description: Python node (error_report_sub) (Omnihand2025Pro)
"""
        初始化电机错误报告订阅节点
        ...
        self.motor_error_report_fb_subscriber = self.create_subscription(
            MotorErrorReport,
            f'/omnihand/omnihand_pro_2025/{hand_side}/motor_error_report',
            self.motor_error_report_callback,
            10
        )
        
        self.get_logger().info(f'{hand_side.capitalize()} Motor Error Report Subscriber Node started')
        self.get_logger().info(f'Subscribing to: /omnihand/omnihand_pro_2025/{hand_side}/motor_error_report')
