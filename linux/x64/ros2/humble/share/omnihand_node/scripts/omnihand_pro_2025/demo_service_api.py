...
@Date: 2025-01-XX
@Description: Demo script showing how to use the simplified Service API for OmniHand Pro 2025 (O12)
...
class HandServiceDemo(Node):
    """Demo class showing how to use the simplified Service API for OmniHand Pro 2025 (O12)"""
    ...
    def __init__(self, hand_side='left'):
        ...
        self.get_logger().info(f'{hand_side.capitalize()} Hand Service Demo Node started (O12)')
    ...
    def run_demo(self):
        ...
        self.get_logger().info('=' * 60)
        self.get_logger().info('OmniHand Pro 2025 (O12) Service API Demo')
        self.get_logger().info('=' * 60)
        self.get_logger().info('')
        ...
        self.get_logger().info('      O12 has 12 active joints (different from O10 which has 10)')
        self.get_logger().info('')
        ...
        # O12 各关节最大角度（弧度）: [0.94, 0, 0, 0, 0.26, 1.35, 1.53, 0.26, 1.36, 1.82, 1.55, 1.54]
        # 转换为度数: [53.9°, 0°, 0°, 0°, 14.9°, 77.4°, 87.7°, 14.9°, 77.9°, 104.3°, 88.8°, 88.3°]
        # 1度对所有关节都在有效范围内
        self.get_logger().info('')
        ...
        # O12 各关节最大角度（弧度）: [0.94, 0, 0, 0, 0.26, 1.35, 1.53, 0.26, 1.36, 1.82, 1.55, 1.54]
        # 转换为度数: [53.9°, 0°, 0°, 0°, 14.9°, 77.4°, 87.7°, 14.9°, 77.9°, 104.3°, 88.8°, 88.3°]
        # 关节1最大只有53.9°，关节2-4为0°（不能动），关节5和8最大只有14.9°
