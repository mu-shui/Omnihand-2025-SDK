...
@Date: 2025-11-06
@Description: Python node for current report (Omnihand2025Pro)
"""
        初始化电流报告订阅节点
        ...
        self.current_report_fb_subscriber = self.create_subscription(
            CurrentReport,
            f'/omnihand/omnihand_pro_2025/{hand_side}/current_report',
            self.current_report_feedback_callback,
            10
        )

        self.get_logger().info(f'{hand_side.capitalize()} Current Report Subscriber Node started')
        self.get_logger().info(f'Subscribing to: /omnihand/omnihand_pro_2025/{hand_side}/current_report')
