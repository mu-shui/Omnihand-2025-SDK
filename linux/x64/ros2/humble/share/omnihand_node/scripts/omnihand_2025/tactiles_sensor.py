...
@Date: 2025-11-06
@Description: Python node for tactile sensor (Omnihand2025)
"""
        初始化触觉传感器订阅节点
        ...
        self.tactile_sensor_subscriber = self.create_subscription(
            TactileSensor,
            f'/omnihand/omnihand_2025/{hand_side}/tactile_sensor',
            self.tactile_sensor_feedback_callback,
            10
        )

        self.get_logger().info(f'{hand_side.capitalize()} Tactile Sensor Subscriber Node started')
        self.get_logger().info(f'Subscribing to: /omnihand/omnihand_2025/{hand_side}/tactile_sensor')
